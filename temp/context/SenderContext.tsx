...
interface SenderStateProps {
  connectWallet: () => void;
  getAllBookings: () => void;
  isLoading: boolean;
}

export const SenderContext = React.createContext({} as SenderStateProps);
...
export const SenderProvider = ({ children }) => {
  ...
  //       const availableTransactions = await senderContract.getAllBookings();
  ...
  const getAllBookings = async () => {
    try {
      ...
      const senderContract = getEthereumContract();
      const bookings = await senderContract.getAllBookings();
  ...
  const sendTransaction = async () => {
    try {
      if (ethereum) {
        ...
        const transaction = await senderContract.transfer(addressTo, message, {
          value: parsedAmount,
        });
